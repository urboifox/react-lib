 ...
 * @param {Yup.Schema} validationSchema - Optional Yup schema for form validation.
 * 
...
function useForm(payload, validationSchema) {
  const [data, setData] = useState(payload); // State to hold form data
  const [errors, setErrors] = useState(payload); // State to hold form validation errors
  const [valid, setValid] = useState(false); // State to track if the form is valid
